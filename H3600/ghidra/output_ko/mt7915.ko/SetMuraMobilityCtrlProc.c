// module: mt7915.ko
// function: SetMuraMobilityCtrlProc @ 0x96224
// size: 688 bytes
//

undefined4 SetMuraMobilityCtrlProc(int param_1,char *param_2)

{
  ...
  if (param_2 == (char *)0x0) {
    ...
    if (sVar2 == 1) {
      ...
      if (0 < DebugLevel) {
        printk("%s(): fgMobilityDetectEn: %d\n","SetMuraMobilityCtrlProc",uVar1);
      }
...
LAB_0009636c:
  if (((0 < DebugLevel) && (printk("Expected format is as below:\n\n"), 0 < DebugLevel)) &&
     (printk("\tiwpriv <interface> set mura_mobility_en=O\n\n"), 0 < DebugLevel)) {
    printk("\tparam1: Mobility Enable (0, 1)\n");
  }
  return 0;
}
