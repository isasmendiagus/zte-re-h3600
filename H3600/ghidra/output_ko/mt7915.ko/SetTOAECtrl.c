// module: mt7915.ko
// function: SetTOAECtrl @ 0xc02a8
// size: 412 bytes
//

undefined4 SetTOAECtrl(undefined4 param_1,char *param_2)

{
  byte bVar1;
  size_t sVar2;
  int iVar3;
  undefined4 uVar4;
  char cVar5;
  
  if (param_2 == (char *)0x0) {
    if (-1 < DebugLevel) {
      printk("%s: No parameters!!\n","SetTOAECtrl");
    }
  }
  else {
    sVar2 = strlen(param_2);
    if (sVar2 == 1) {
      iVar3 = rstrtok(param_2,&_LC156);
      if (iVar3 == 0) {
        bVar1 = 0;
      }
      else {
        bVar1 = 0;
        cVar5 = '\0';
        do {
          if (cVar5 == '\0') {
            bVar1 = os_str_tol(iVar3,0,10);
            iVar3 = rstrtok(0,&_LC156);
            cVar5 = '\x01';
            if (iVar3 == 0) break;
          }
          if (-1 < DebugLevel) {
            printk("%s: set wrong parameters\n","SetTOAECtrl");
          }
          cVar5 = cVar5 + '\x01';
          iVar3 = rstrtok(0,&_LC156);
        } while (iVar3 != 0);
        if (1 < bVar1) {
          if (DebugLevel < 0) {
            return 0;
          }
          printk("%s: Please input 1(Enable) or 0(Disable)!!\n","SetTOAECtrl");
          return 0;
        }
      }
      if (0 < DebugLevel) {
        printk("%s: TOAECtrl = %d\n","SetTOAECtrl",bVar1);
      }
      uVar4 = TOAECtrlCmd(param_1,bVar1);
      return uVar4;
    }
    if ((-1 < DebugLevel) &&
       (printk("%s: Wrong parameter format!!\n","SetTOAECtrl"), -1 < DebugLevel)) {
      printk("%s: Please use input format like X (X = 0,1)!!\n","SetTOAECtrl");
    }
  }
  return 0;
}

