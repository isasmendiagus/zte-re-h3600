// module: mt7915.ko
// function: MtCmdThermalProtectDisable @ 0x1b51a4
// size: 348 bytes
//
...

{
  ...
  local_38 = 0;
  if ((0 < DebugLevel) &&
     (printk("%s: band_idx: %d, protect_type: %d\n","MtCmdThermalProtectDisable",param_2,param_3),
     0 < DebugLevel)) {
    printk("%s: trigger_type: %d\n","MtCmdThermalProtectDisable",param_4);
  }
  ...
  if (3 < DebugLevel) {
    printk("%s:(ret = %d)\n","MtCmdThermalProtectDisable",uVar2);
  }
  return uVar2;
}
