// module: mt7915.ko
// function: SetATEIBfProfileUpdate @ 0x267228
// size: 1424 bytes
//

undefined4 SetATEIBfProfileUpdate(int param_1,char *param_2)
...
LAB_0026728c:
    ...
    if (-1 < DebugLevel) {
      ...
      printk("%s : band[%d]\'s TxAntennaSel = 0x%x\n","SetATEIBfProfileUpdate",uVar6,
             *(undefined4 *)(uVar6 * 0xd18 + iVar5 + 0x1128));
    }
    iVar5 = net_ad_wrap_service(param_1);
    ...
    Set_TxBfProfileTag_BfType(param_1,&_LC326);
    Set_TxBfProfileTag_DBW(param_1,&_LC326);
    Set_TxBfProfileTag_SuMu(param_1,&_LC326);
    mt_WrapIBfCalGetIBfMemAlloc(param_1,local_88,local_88 + 2);
    ...
    Set_TxBfProfileTag_TimeOut(param_1,&_LC331);
    Set_TxBfProfileTag_DesiredBW(param_1,&_LC326);
    snprintf(local_78,0x50,"%d",local_98);
    Set_TxBfProfileTag_DesiredNr(param_1,local_78);
    snprintf(local_78,0x50,"%d",uVar9);
    Set_TxBfProfileTag_DesiredNc(param_1,local_78);
    ...
    snprintf(local_78,0x50,
             "%.2x:00:%.2x:00:00:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x"
             ,uVar3,uVar8,(uint)(byte)(&UNK_002a58e0)[local_9c],uVar9,local_98,local_88[0] & 0xff,
