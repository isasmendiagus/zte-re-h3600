...

{
  ...
  if (_tm_port_unknwn_multicast_floodport_set != 0) {
    uVar6 = 0;
    puVar9 = g_sw_netInfo;
    pcVar8 = zte_api_sw_mac_set_port_macfiltermode;
    iVar10 = 0;
    uVar7 = 1;
    do {
      uVar2 = (undefined1)uVar6;
      if (uVar6 < 8) {
        if ((byte)*pcVar8 == _tm_vlan_check_ena_set) {
          iVar10 = dev_get_by_name(&init_net,0x2b66c);
          printk("port id %d, name %s\n",_tm_vlan_check_ena_set,0x2b66c);
        }
        else {
