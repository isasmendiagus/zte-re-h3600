...
/* ============================================================
 *   Per-block stock-init replay helper (refactor #38 Phase 6)
 *
 * Each HW sub-block has a contiguous slice of zx_stock_init_table[]
 * delimited by ZX_STOCK_<name>_{START,END} constants generated by
 * tasks/00.01.eth-driver/scripts/annotate_stock_table.py.
 *
 * Callers iterate per-block (PON_LOW, PON_B, PON_TAIL, NPP, NPP_AUX,
 * TM, PP_FUC) instead of one big 22k-entry loop. Same writes; nameable
 * stages in dmesg. Stepping stone to splitting each block out of the
 * table entirely into its own zx_<block>_init() function with explicit
 * register-by-register code.
 * ============================================================ */
static void zx_replay_stock_block(struct zx_eth *e, const char *name,
				   u32 start, u32 end)
{
	u32 i, neg = 0, pos = 0;
	...
	for (i = start; i < end; i++) {
		...
		if (off < 0) {
			writel(val, e->pon_early + (off + 0x1C0000));
			neg++;
		} else {
			writel(val, e->base + off);
			pos++;
	...
	dev_info(e->dev, "stock-init %s: %u regs (%u pon_early + %u base)\n",
		 name, end - start, neg, pos);
}
...
static int zx_eth_probe(struct platform_device *pdev)
{
	...
	/* Refactor #38 Phase 6: replay stock-init in per-HW-block stages.
	 * Each block is a slice of zx_stock_init_table[]; the helper logs
	 * per-block counts so dmesg shows the init sequence stage-by-stage.
	 * Same total writes as the old monolithic loop, just nameable
	 * stages. Future phases will replace each call with a real
	 * zx_<block>_init() function deriving values parametrically. */
	zx_replay_stock_block(eth, "PON_LOW",  ZX_STOCK_PON_LOW_START,  ZX_STOCK_PON_LOW_END);
	zx_replay_stock_block(eth, "PON_B",    ZX_STOCK_PON_B_START,    ZX_STOCK_PON_B_END);
	zx_replay_stock_block(eth, "PON_TAIL", ZX_STOCK_PON_TAIL_START, ZX_STOCK_PON_TAIL_END);
	zx_replay_stock_block(eth, "NPP",      ZX_STOCK_NPP_START,      ZX_STOCK_NPP_END);
	zx_replay_stock_block(eth, "NPP_AUX",  ZX_STOCK_NPP_AUX_START,  ZX_STOCK_NPP_AUX_END);
	zx_replay_stock_block(eth, "TM",       ZX_STOCK_TM_START,       ZX_STOCK_TM_END);
	zx_replay_stock_block(eth, "PP_FUC",   ZX_STOCK_PP_FUC_START,   ZX_STOCK_PP_FUC_END);
