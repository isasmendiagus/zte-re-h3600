...

{
  ...
  if ((g_pon_work_mode & 0x10) != 0) {
    if (wanLogicPort == 0xff) {
      if (wan_to_lan != 0) {
        uVar1 = (uint)*(byte *)(param_1 + 0xbc);
LAB_0001083c:
        if (_zte_api_sw_mac_set_port_learnlimit == uVar1) {
          iVar3 = dev_get_by_name(&init_net,&_LC1,iVar3,_zte_api_sw_mac_set_port_learnlimit,param_4)
          ;
      ...
      uVar1 = (uint)*(byte *)(param_1 + 0xbc);
      if ((byte)tm_set_p2pmode[wanLogicPort] != uVar1) goto LAB_0001083c;
      iVar3 = dev_get_by_name(&init_net,&_LC0,iVar3,(uint)(byte)tm_set_p2pmode[wanLogicPort],param_4
                             );
joined_r0x000108fc:
      if (iVar3 == 0) {
