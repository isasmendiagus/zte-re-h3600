// module: switch.ko
// function: sw_mac_del_port_macaddress @ 0x14a74
// size: 288 bytes
//
...

{
  ...
  if (2 < g_switch_debug_level) {
    printk("[%s] input:\n%s = %d\n%s = %2x:%2x:%2x:%2x:%2x:%2x\n%s = %d\n%s = %d\n",
           "sw_mac_del_port_macaddress","MacEntry.port",*param_1,"MacEntry.mac",(char)param_1[1],
  ...
  uVar4 = 0;
  if (iVar3 != 0) {
    printk("[%s] L%d tm sdk failed!\n","sw_mac_del_port_macaddress",0xb1);
    uVar4 = 0xffffffff;
  }
  return uVar4;
}
