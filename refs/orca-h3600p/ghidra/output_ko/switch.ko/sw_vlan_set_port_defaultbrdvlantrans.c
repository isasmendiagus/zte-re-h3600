// module: switch.ko
// function: sw_vlan_set_port_defaultbrdvlantrans @ 0x1aeec
// size: 172 bytes
//
...

{
  ...
  if (uVar2 < _tm_port_unknwn_multicast_floodport_set) {
    iVar1 = tm_port_dft_broadcst_vltrans_set(tm_set_p2pmode[uVar2],uVar3);
  ...
  printk("[%s]%d input port error!\n","sw_vlan_set_port_defaultbrdvlantrans",0x216);
  return -1;
}
