// module: switch.ko
// function: sw_port_get_port_ratelimit @ 0x199d0
// size: 292 bytes
//
...

{
  ...
  if (2 < g_switch_debug_level) {
    printk("[%s] input:\n%s = %d\n","sw_port_get_port_ratelimit","RateLimit.port",cVar1);
  }
  ...
  if (iVar3 == 0) {
    ...
    if (2 < uVar2) {
      printk("[%s] output:\n %s = %d \n %s = %d \n %s = %d kbps\n %s = %d\n",
             "sw_port_get_port_ratelimit","RateLimit.direct",(uint)local_1f,"RateLimit.enable",
             (uint)local_1e,"RateLimit.rate",local_1c[0],"RateLimit.ifgmode",(uint)local_1d);
    }
  }
  else {
    printk("[%s] L%d tm sdk failed!\n","sw_port_get_port_ratelimit",0x50);
  }
  return iVar3;
}
