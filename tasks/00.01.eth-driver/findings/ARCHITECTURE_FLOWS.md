...
|---|---|---|---|---|
| 1 | eth.c:1916-1921 | **Ethertype offset detect** — `frame_at = (bp_buf[12..13] >= 0x0600 && != 0xffff) ? bp_buf : bp_buf+16` | 🔴 **NOT in stock**. Stock `pon_tm_net_poll` memcpy's from one fixed pointer (`pvVar12`) with NO conditional. Stock doesn't need it because its HW FDB is correct → switch never floods CPU TX back → no `+0` looped frames to disambiguate. We invented this for our broken-flood state. **Should become a no-op (always +16) once HW FDB is correctly populated.** | We observed RX buffers with frame at +0 (loopback bounces) AND +16 (fresh RX). Couldn't tell apart by length/desc. Used ethertype as crude tag. |
