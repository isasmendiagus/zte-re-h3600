...
#
# ┌──────────────────────────────────────────────────────────────────────┐
# │ ⚠️  STALE — DESIGN REFERENCE ONLY, DO NOT RUN AS-IS (2026-05-23)    │
# │                                                                      │
# │ This script was written before the H3600/→zxic/ rename and before   │
# │ we standardized on the soft-float toolchain. Three blockers:        │
# │                                                                      │
# │   1. CROSS_COMPILE=arm-linux-gnueabihf-  (HARD-FLOAT)                │
# │      → change to arm-linux-gnueabi- (see docs/CROSS_COMPILE_GUIDE.md)│
# │                                                                      │
# │   2. TFTP_DIR=/srv/tftp                                              │
# │      → change to /home/ubuntu/Projects/MYSELF/ZTE/zxic/tftp/         │
# │                                                                      │
# │   3. LOADADDR 0x40008000                                             │
# │      → our active mainline iter uses 0x42000000; re-verify which     │
# │        applies for OpenWrt's slot-A boot path                        │
# │                                                                      │
# │ Activate this script only when starting stage 5/6 (OpenWrt port)    │
# │ work. Until then it's a design reference for the build pipeline.    │
# │ See tasks/00.openwrt-port/README.md for context.                    │
# └──────────────────────────────────────────────────────────────────────┘
#
# Workflow (when reactivated):
...
#
# Output: /srv/tftp/uImage (STALE — see above)

set -euo pipefail

echo "ERROR: This script is STALE — see header. Refusing to run." >&2
echo "       Fix the three blockers documented at the top, then" >&2
echo "       remove this guard before invoking." >&2
exit 1
