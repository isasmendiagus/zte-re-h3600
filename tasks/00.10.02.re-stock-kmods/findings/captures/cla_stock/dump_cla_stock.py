...
def rd(ram,addr):
    ...
    run('fpga -r 0x73000 1 >/dev/null 2>&1',0.5)   # warmup
...
plan=[(0,range(0,0x11)),(1,range(0,0x30)),(2,range(0,0x18)),(3,range(0,0x18)),(4,range(0,0x18)),(5,range(0,0x18)),(6,range(0,0x18))]
for ram,rng in plan:
    for a in rng:
        d=rd(ram,a)
        if any(x not in ('00000000','????????') for x in d):
            print('ram%d a0x%02x: %s'%(ram,a,' '.join(d)),flush=True)
ch.close(); c.close()
